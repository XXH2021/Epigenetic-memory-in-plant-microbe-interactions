library("ggplot2")
library(pheatmap)
data = read.csv("diff_cog_all_anno.csv", header=TRUE, row.names = 2)
data<-t(data[,8:25])
...
write.csv(fun.count,"cog_all.csv")

cato.5<-levels(annotation_row$COGClass)[table(annotation_row$COGClass)<50]
cato.6<-levels(annotation_row$COGClass)[table(annotation_row$COGClass)>50]
...
cbPalette <- c("#E69F00", "#56B4E9", "#009E73", "#F0E442", "#0072B2", "#D55E00", "#CC79A7","red", "pink","#000000")
...
pheatmap(mat_mean, scale = "row", annotation_row = annotation_row,annotation_colors=annotation_colors,border=FALSE,
	...
	width = 5, height = 11,filename = "heatmap_diff_cog_day3_day30.pdf")
