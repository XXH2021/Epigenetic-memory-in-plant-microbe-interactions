library("ggplot2")
library(pheatmap)
data = read.csv("diff_cog_CK_PGP53_anno.csv", header=TRUE, row.names = 2)
data<-t(data[,8:25])
...
write.csv(fun,"cog3.pgp5.fun.csv")
...
write.csv(fun.count,"cog3_pgp5.csv")
cato.5<-levels(annotation_row$COGClass)[table(annotation_row$COGClass)<5]
cato.6<-levels(annotation_row$COGClass)[table(annotation_row$COGClass)>4]
...
annotation_colors<-list(COGClass=da[[1]]) 
   cellwidth = 20, cellheight = 4, fontsize = 7,treeheight_row=15,treeheight_col =15,show_rownames=F,color = colorRampPalette(c("red", "white", "blue"))(50))
...
pheatmap(mat_mean, scale = "row", annotation_row = annotation_row,annotation_colors=annotation_colors,border=FALSE,
	...
	width = 4, height = 7,filename = "heatmap_diff_cog_3_pgp5_.pdf")
